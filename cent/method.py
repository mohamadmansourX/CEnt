...
class CEnt(RecourseMethod):
    ...
    def explain_in_text(self, factual):
        target_class = factual[self._mlmodel.data.target].values[0]
        flipped_class = target_class * -1 + 1

        print("To flip the binary class from {} to {}, we moved our factual to a point satisfying the following conditions:".format(target_class, flipped_class))
        for rule in self.found_node.conditions:
            feature = rule.feature
            if feature in self.data_catalog['categorical']:
                to_bin_value = bool(factual[feature].values[0]*-1+1)
                print(f'\t{feature} needs to be flipped to {to_bin_value}')
            else:
                print(f'\t{feature} needs to be {rule.threshold_sign} than {rule.threshold}')
